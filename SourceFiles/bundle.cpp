...
bool ZAppBundle::SignNode(JValue &jvNode)
{
    ...
    // Code Contrib
    // By github/@camila314 - From: https://github.com/zhlynn/zsign/pull/301
...
bool ZAppBundle::SignFolder(ZSignAsset *pSignAsset,
                            ...
                            bool bInjectToFrameworks) // تمت إضافة البارامتر الأخير
{
    m_bForceSign = bForce;
                ...
                for (size_t i = 0; i < arrPlugIns.size(); i++)
                {
                    ...
                    if (jvPlugInInfoPlist.readPListPath("%s/Info.plist", strPlugin.c_str()))
                    {
                        ...
                        if (jvPlugInInfoPlist.has("NSExtension") &&
                            jvPlugInInfoPlist["NSExtension"].has("NSExtensionAttributes") &&
                            jvPlugInInfoPlist["NSExtension"]["NSExtensionAttributes"].has("WKAppBundleIdentifier"))
                        {
    ...
    if (!strDyLibFile.empty())
    {
        ...
        if (!strDyLibData.empty())
        {
            ...
            if (bInjectToFrameworks) {
                // إنشاء مجلد Frameworks إذا لم يكن موجودًا
                string strFrameworksPath = m_strAppFolder + "/Frameworks";
                CreateFolder(strFrameworksPath.c_str());
                
                strTargetPath = strFrameworksPath + "/" + strFileName;
                StringFormat(m_strDyLibPath, "@executable_path/Frameworks/%s", strFileName.c_str());
            } else {
                strTargetPath = m_strAppFolder + "/" + strFileName;
                StringFormat(m_strDyLibPath, "@executable_path/%s", strFileName.c_str());
            }
            
            if (WriteFile(strDyLibData, strTargetPath.c_str())) {
